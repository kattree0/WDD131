...
function renderRecipes(recipeList) {
    ...
    const recipeHTML = document.querySelector(".recipe-html");

    const recipe = recipeList[0];
	// use the recipeTemplate function to transform our recipe objects into recipe HTML strings
let innerhtml = recipeTemplate(recipe);
	// Set the HTML strings as the innerHTML of our output element.
    recipeHTML.innerHTML = innerhtml;
...
function resetGridLayout() {
    const recipeHtml = document.querySelector('.recipe-html');
    recipeHtml.style.display = 'grid'; // Ensure the grid is still applied
    recipeHtml.style.gridTemplateColumns = '1fr 1fr'; // Ensure the columns are still 2
}

// Call this after adding/removing elements dynamically
resetGridLayout();
...
function filterRecipes(query) {
    const filteredRecipes = recipes.filter(recipe => {
        const match = recipe.name.toLowerCase().includes(query);
        return match;  });
...
function renderRecipe(recipes) {
    const recipeContainer = document.querySelector(".recipe-html"); // Ensure you have a container in your HTML
    recipes.forEach((recipe,index) => {
        const recipeElement = document.createElement('section');
        recipeElement.classList.add('recipe-html');
        renderRecipes([index]);
    });
